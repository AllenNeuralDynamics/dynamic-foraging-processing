...
| 2026-09-21 | `auto_waterL` / `auto_waterR` and `anti_bias_left_water` / `anti_bias_right_water` are now `0` / `False` on trials whose go-cue water came from the experimenter. `TrialOutcome` reports manual go-cue-aligned water as ordinary autowater (`is_autowater` is `True` and `is_auto_reward_right` points to the watered side), so the `{Left,Right}ManualAutoReward` streams are the only way to tell it from water the task scheduled. Each event is resolved to the valve opening it caused and then to that opening's trial — the event fires mid-trial, one or more trials before the water lands, so it cannot be matched to a trial directly. This makes the columns agree with the reward-delivery series, which already labels those deliveries `manual_go_cue_aligned`: on 872547_2026-09-17 trials 276 / 277 the table claimed autowater for water the experimenter gave. Sessions with no `ManualAutoReward` stream — every session before 2026-09-17 — are unchanged. |
