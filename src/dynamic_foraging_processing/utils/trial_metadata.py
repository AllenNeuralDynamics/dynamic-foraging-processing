...
def get_bias_metadata(trial: Trial) -> BlockBasedTrialMetadata:
    ...
    the anti-bias water intervention share that channel and are told apart here,
    by ``is_autowater`` and ``is_bias_water_intervention``.
    (``is_bias_stage_intervention`` marks the anti-bias algorithm's other lever,
    moving the lickspouts.)
